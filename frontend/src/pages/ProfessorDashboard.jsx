      ...
      <div className="card" style={{ background: 'linear-gradient(135deg,#faf5ff 0%,#e9d8fd 100%)', border: '1px solid #d6bcfa', marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px' }}>
            {[
              [activeBorrows.length + '/10', 'Active Borrows',  '#553c9a'],
              ...
              <tbody>
                {preborrows.length === 0 && (
                  <tr><td colSpan={6} style={{ textAlign: 'center', color: '#718096', padding: '28px' }}>
                    No reservations yet. If a book has no available copies, reserve it from <strong>Browse &amp; Borrow</strong>.
                  </td></tr>
                )}
                {preborrows.map(p => (
                  <tr key={p.id}>
                    ...
                    <td>
                      {p.status === 'READY' && (
                        <button className="btn btn-success btn-sm" onClick={() => handleFulfil(p.id)}>Fulfil →</button>
                      )}
                      {p.status === 'PENDING' && (
                        <button className="btn btn-danger btn-sm" onClick={() => handleCancelReserve(p.id)}>Cancel</button>
                      )}
