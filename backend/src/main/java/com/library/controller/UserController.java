...
public class UserController {
    ...
    @GetMapping("/students")  public List<Student> getAllStudents() { return userService.getAllStudents(); }
    @GetMapping("/faculty")   public List<Faculty> getAllFaculty()  { return userService.getAllFaculty();  }
    ...
    public ResponseEntity<?> updateStudent(@PathVariable Long id, @RequestBody Student s) {
        try   { return ResponseEntity.ok(userService.updateStudent(id, s)); }
        catch (RuntimeException e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }

    @PutMapping("/faculty/{id}")
    public ResponseEntity<?> updateFaculty(@PathVariable Long id, @RequestBody Faculty f) {
        try   { return ResponseEntity.ok(userService.updateFaculty(id, f)); }
        catch (RuntimeException e) { return ResponseEntity.badRequest().body(e.getMessage()); }
    }
